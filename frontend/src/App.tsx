...
export default function App() {
  ...
  // Load from real Java Backend on Mount
  useEffect(() => {
    async function loadBackendData() {
      try {
        const [backendJobs, backendCandidates, backendQuestions, backendAuditLogs, backendNotifs] = await Promise.all([
          api.getJobs(),
          api.getCandidates(),
          api.getQuestions(),
          api.getAuditLogs(),
        ...
        if (backendJobs && backendJobs.length > 0) {
          setJobs(backendJobs);
          setSelectedJob(backendJobs[0]);
        }
        if (backendCandidates && backendCandidates.length > 0) {
          setCandidates(backendCandidates);
          setSelectedCandidate(backendCandidates[0]);
          setCandidateA(backendCandidates[0]);
          if (backendCandidates.length > 1) {
            setCandidateB(backendCandidates[1]);
          }
        }
        ...
        console.warn("Could not load backend data, staying with local initial state:", err);
      }
    }
    loadBackendData();
  }, []);
  ...
  const handleSignInSuccess = async (role: UserRole, email: string) => {
    setCurrentRole(role);
    if (role === 'candidate') {
      setActiveTab('candidate_portal');
  ...
  const handleSaveWeights = (weights: JobRequisition['weights']) => {
    ...
    api.updateWeights(selectedJob.id, weights).catch(() => {});
  };
  ...
  const handleCandidateAdded = (newCand: Candidate) => {
    ...
    api.createCandidate(newCand).catch(() => {});
  };
  ...
  const handleMoveCandidateStage = (candidateId: string, newStatus: CandidateStatus) => {
    ...
    api.updateCandidateStatus(candidateId, newStatus).catch(() => {});
  };
  ...
  const handleScheduled = (candidate: Candidate, date: string, type: string) => {
    ...
    api.recordAuditLog(newLog).catch(() => {});
  };
