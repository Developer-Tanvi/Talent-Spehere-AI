...
2. Distributed Transactions: "How do you handle eventual consistency between Kafka event consumers and PostgreSQL read replicas?"`,
  ...
  const handleSend = async (textToSend?: string) => {
    ...
    try {
      const apiRes = await api.askCopilot(text, candidateElena?.id, selectedJob?.id);
      let aiText = apiRes?.response || `Based on multi-evidence evaluation for ${selectedJob.title}: Candidate profiles show strong alignment with core Java and distributed systems. Verified GitHub artifacts and OA results support moving top candidates to the technical interview loop.`;
